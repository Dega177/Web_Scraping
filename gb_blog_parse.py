...
class GbBlogParseWithComments(GbBlogParse):
    ...
    def parse_post(self, response: requests.Response):
        ...
        commentable_id = int(soup.find(name='comments')['commentable-id'])
        data = {
            'post_data': {
                'id': commentable_id,
                'url': response.url,
            ...
            'comments_data': None if comments_cnt == '0' else self.parse_post_comments(commentable_id, comments_cnt)
        }
        self._save(data)
    ...
    def parse_post_comments(self, commentable_id, comments_cnt):
        ...
        def get_comments(comments):
            ...
            for comment in comments:
                ...
                if 'children' in comment['comment']:
                    data += get_comments(comment['comment']['children'])
            return data
